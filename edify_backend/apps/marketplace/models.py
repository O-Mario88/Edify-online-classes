...
class PesapalTransaction(models.Model):
    ...
    user = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name='pesapal_transactions')
    ...
    currency = models.CharField(max_length=10, default='UGX')
    payment_method = models.CharField(max_length=20, default='PESAPAL')
    ...
    def __str__(self):
        return f"{self.user.email} - {self.merchant_reference} ({self.state})"
