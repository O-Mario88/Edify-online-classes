       ...
       <CardContent className="p-5">
          <div className="grid grid-cols-3 gap-4 mb-6">
             <div className="p-4 bg-blue-50/50 rounded-lg border border-blue-100 text-center">
                ...
                <span className="block text-xs font-bold uppercase text-blue-500 mt-1">Tracked Alumni</span>
             </div>
             <div className="p-4 bg-green-50/50 rounded-lg border border-green-100 text-center">
                ...
                <span className="block text-xs font-bold uppercase text-green-500 mt-1">University Rate</span>
             </div>
             <div className="p-4 bg-purple-50/50 rounded-lg border border-purple-100 text-center">
                ...
                <span className="block text-xs font-bold uppercase text-purple-500 mt-1">Direct Hires</span>
             </div>
          </div>

          <h4 className="text-xs font-bold uppercase text-slate-400 mb-3 tracking-wider">Recent Verifiable Outcomes</h4>
                ...
                <div key={i} className="flex justify-between items-center p-3 rounded-lg border border-slate-100 bg-slate-50/50 hover:bg-slate-100 transition-colors">
                   <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded bg-white shadow-sm border border-slate-200 flex items-center justify-center shrink-0">
                         {i === 1 ? <Briefcase className="w-5 h-5 text-purple-500" /> : <Building className="w-5 h-5 text-green-500" />}
                      </div>
                      <div>
                         <p className="font-bold text-slate-800 text-sm">
                           {alum.name} <span className="text-slate-400 font-normal">({alum.year})</span>
                         </p>
                         <p className="text-xs text-slate-600 font-medium">{alum.path} • {alum.degree}</p>
                      </div>
                   </div>
                   <div className="text-right">
                      <Badge variant="outline" className="text-[10px] text-slate-500 bg-white shadow-sm font-semibold border-slate-200">
                        {alum.school}
                      </Badge>
                   </div>
                </div>
