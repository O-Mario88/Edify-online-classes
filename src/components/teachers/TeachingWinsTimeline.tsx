...
export const TeachingWinsTimeline: React.FC<TeachingWinsTimelineProps> = ({ wins }) => {
  const getIcon = (type: string) => {
    switch (type) {
      ...
      case 'payout_qualified': return <DollarSign className="w-4 h-4 text-amber-800" />;
      case 'resource_popular': return <Zap className="w-4 h-4 text-blue-800" />;
      case 'badge_earned': return <Award className="w-4 h-4 text-purple-800" />;
      case 'attendance_improved': return <Users className="w-4 h-4 text-cyan-600" />;
      case 'intervention_success': return <CheckCircle className="w-4 h-4 text-emerald-800" />;
      default: return <Trophy className="w-4 h-4 text-indigo-800" />;
    }
  };
  ...
  return (
    <Card className="shadow-sm border-slate-200 h-full">
      ...
      <CardContent className="p-4">
        {wins.length === 0 ? (
          <p className="text-sm text-slate-700 italic text-center mt-4">No recent wins logged. Keep up the great teaching!</p>
        ) : (
            ...
            {wins.map((win, idx) => (
              <div key={win.id} className="relative pl-6">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-800 mt-1 leading-relaxed">{win.description}</p>
                  <p className="text-[10px] text-slate-800 font-medium tracking-wide mt-1.5 uppercase">{win.timestamp}</p>
                </div>
              </div>
            ))}
          </div>
