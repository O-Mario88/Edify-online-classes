...
export const SmartInterventionBuilder: React.FC<{ data: InterventionBuilderData }> = ({ data }) => {
  ...
  const getIcon = (type: string) => {
    switch (type) {
      case 'video': return <Video className="w-4 h-4 text-blue-700" />;
      case 'note': return <FileText className="w-4 h-4 text-orange-500" />;
      case 'quiz': return <CheckCircle className="w-4 h-4 text-emerald-700" />;
      case 'discussion': return <HelpCircle className="w-4 h-4 text-purple-700" />;
          ...
          <div className="flex justify-between items-start">
             <div>
               <CardTitle className="flex items-center gap-2 text-md text-slate-800">
                  <Package className="w-5 h-5 text-indigo-800" />
                  Smart Intervention Builder
               </CardTitle>
               <CardDescription className="text-slate-700 text-xs mt-1">1-Click targeted resource bundling</CardDescription>
             </div>
       ...
       <CardContent className="p-4 flex-1 flex flex-col justify-between">
          <div>
             <div className="flex justify-between items-center mb-2">
                ...
                <Button variant="ghost" size="sm" className="h-6 text-indigo-800 text-xs p-0 px-2" onClick={() => setIsEditing(!isEditing)}>
                ...
                {resources.filter(r => isEditing || r.isIncluded).map((res) => (
                  <div key={res.id} className={`flex justify-between items-center p-2 rounded-md border transition-colors ${res.isIncluded ? 'bg-slate-50 border-indigo-100' : 'bg-white border-slate-100 opacity-60'}`}>
                     <div className="flex items-center gap-2">
                        {getIcon(res.type)}
                        <span className={`text-xs font-semibold ${res.isIncluded ? 'text-slate-700' : 'text-slate-800 line-through'}`}>{res.title}</span>
                     </div>
                     {isEditing && (
                       <button onClick={() => handleToggle(res.id)} className={`w-5 h-5 flex justify-center items-center rounded-full border ${res.isIncluded ? 'border-red-200 text-red-700 hover:bg-red-50' : 'border-green-200 text-emerald-800 hover:bg-green-50'}`}>
