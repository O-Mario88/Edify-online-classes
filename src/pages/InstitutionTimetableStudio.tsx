...
const EMPTY_GRID = () => DAYS.map(() => PERIODS.map((p) => p.includes('Lunch') ? 'LUNCH' : ''));

export function InstitutionTimetableStudio() {
  const [activeTab, setActiveTab] = useState('master-grid');
  const [timetableGrid, setTimetableGrid] = useState<string[][]>(EMPTY_GRID());
  ...
  return (
    <PremiumGate
      requiredTier="termly"
      featureName="Timetable Studio"
      description="The Master Timetable orchestrates automatic scheduling, cover arrangements, and real-time conflict management. Upgrade your institution to the Term package to deploy intelligent timetabling."
    >
                   ...
                   <tbody className="divide-y divide-slate-100 dark:divide-slate-800/50">
                     {DAYS.map((day, rowIndex) => (
                       <tr key={day} className="hover:bg-slate-50 dark:hover:bg-slate-900/20 transition-colors">
                         ...
                         {PERIODS.map((_period, colIndex) => {
                           const displayCell = timetableGrid[rowIndex]?.[colIndex] || "";
